Shader "raymarching/molten_para_wp"
{
    Properties
    {
        ...
        SubShader
        {
            ...
            Pass
            {
                ...
                dOut dist(float3 ip)
                {
                    ...
                    //オブジェクトのoriginに球を描画
                    float3 objOrigin = mul(unity_WorldToObject, float4(0,0,0, 1.)).xyz;
                    pos1 =  mul(unity_WorldToObject, float4(pos1,1)).xyz;
                    
                    float3 pos2 = ip + float3(0,0,1);//*_Scale;
                    float size = _Size;
                   // float s = sdSphere(pos1,size);
                    float s = sdSphere(pos1,_Size);
                    
                    s = distance(pos1 * abs(_Scale/2.),objOrigin) - _Size;
                    
                    float bb = sdBox(pos2,float3(100,100,1.1),_walloffset);
                    ...
                    // mul(unity_WorldToObject, float4(pos1,1)).xyz;
                    o.sCenter =  mul(unity_ObjectToWorld, float4(objOrigin,1)).xyz;
                    //o.sCenter = objOrigin;
                    o.sCenter = -(objOrigin - pos1 * abs(_Scale/2.));//今の向きはこっち
                ...
                raymarchingOut inraymarching(raymarchingOut ri)
                {
                    ...
                    for (int i = 0; i < 90; i++)
                    {
                        ...
                        if (d < 0.001)
                        {
                            ...
                            normal = getNormal(rp, float3(0, 0, 15));
                ...
                raymarchingOut raymarching(float4 screen,float3 worldp,float3 inormal)
                {
                   ...
                   if (uvmap.x > 0) {
                        ...
                        if(d.id <1 && (d.bCenter.z+(d.bCenter.z/2)  - d.sCenter.z) > 0.5 
                            && length(d.sCenter.xyz-rp.xyz) < distance(d.sCenter.z,d.bCenter.z)) {
                            ro = inraymarching(ro);
                        }
                    }
                    return ro;
