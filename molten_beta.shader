Shader "raymarching/objspace_plane_cube_rayInray_gradiate"
{
	Properties
	{
		...
		SubShader
		{
			...
			Pass
			{
				...
				dOut dist(float3 ip)
				{
					...
					float3 pos2 = ip - float3(0,0,1) *abs(_Scale/2);
					float size = _Size;
					float s = sdSphere(pos1,size);
					float bb = sdBox(pos2,float3(1,1,0.1) *abs(_Scale/2));
				...
				raymarchingOut raymarching(float4 screen,float3 worldp,float3 inormal)
				{
					...
					if (uvmap.x > 0) {
						...
						if(d.id <1 && (d.bCenter.z - d.sCenter.z) > 0.5 
							&& -length(d.sCenter.xy)+9. > (d.sCenter.z-d.bCenter.z)) {
							ro = inraymarching(ro);
						}
					}
					return ro;
